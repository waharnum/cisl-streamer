...
async function serverStart () {
    ...
    const epubsDirectoryFiles = fs.readdirSync("epubs");
    const epubFilenames = epubsDirectoryFiles.filter(filename => filename.includes(".epub"));
    const epubFilePaths = [];
    epubFilenames.forEach(function (filePath) {
        epubFilePaths.push("epubs/" + filePath);
    });
    console.log(epubFilePaths);

    const publicationURLs = server.addPublications(epubFilePaths);
